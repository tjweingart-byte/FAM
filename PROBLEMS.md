...
## 88. The square arrived; the drawing never did — and the house style never left the laptop

Two reports in one session, both of the form "the picture is not there", and
they had nothing in common except that shape. Worth writing down together for
exactly that reason: the first was a layer that was never built, the second was
three files that were never committed, and neither was a bug in the drawing
pipeline — which is where both looked like they were.

### The preview drew a frame and called it a picture

Reported: on the phone preview the ivory square appears on the player, the
audio runs, and no line is ever drawn; on myFAM, no tile carries one either.

Checked against the real server first, because "it does not draw" is also what
a broken pipeline looks like. It does draw: `tools/visual_probe.py` passes
end to end against a local server with `VISUAL_IMAGE_PROVIDER=synthetic` (the
square starts blank, the reveal advances with the audio, a seek moves it both
ways, pausing freezes it, the player's path is the stored path, Explore has
none), and a browser on `/api/myfam` renders a line thumbnail on 12 of 12
tiles. So the server was never the subject of the report.

The cause was in the two preview builds. §84 added the canvas to
`static/index.html` and inlined `fam-line.js`, so the *client* half shipped
into the preview and mounted a square. Neither shim answers `/api/visual`:
`build_preview.py` fell through to `{"error": "Not available in the preview
build."}` with a 404, `fam-line.js` treated that as "not yet" and polled it
every 2.5 seconds until its ceiling, and no fixture ever carried a `visual`
field for a tile to render. The preview showed the one part of the feature
that needs no server and none of the part that does — and it looked like a
regression in the feature rather than a hole in the stand-in.

Fixed by giving the preview a drawing layer of its own, next to the one that
already stands in for the voice. `FamPreviewArt` (in `build_preview.py`,
injected into both builds) answers `/api/visual`, `/api/visual/event` and
`/api/visual/<id>.svg`, and attaches a finished drawing to every myFAM,
next-up and Explore New row. The figure is `visual_provider._figure` ported to
the browser — the same wound harmonic curve a server with no image credential
draws — because a published page has no image model, exactly as it has no
Claude and no Chatterbox. `fam-line.js` is untouched; the reveal, the tile and
the exclusions are the shipped ones.

Two things it models rather than flattening, because they *are* the feature: a
tile's drawing is already finished when the feed arrives (the browse surfaces
know what might be tapped before it is tapped), and a typed search's drawing
arrives a few seconds late and joins the episode wherever the audio has got
to. Everything it produces carries `placeholder: true`.

Which exposed a smaller thing that was already wrong on the server.
`tools/demo_preflight.py` says synthetic art is "labelled as such on every tile
and player", and only the player was true — `seedThumb` rendered the geometry
with no mark at all. A stand-in labelled in one of the two places it appears is
§51 again, so the tile now carries the same label the player does.

The smoke suite gains one behaviour: *The audio draws the line, and the tiles
carry it*. The existing canvas check could not have caught any of this — the
square it asserts is precisely what a missing drawing layer still gets right.

### The house style was on a laptop and in no commit

Reported from the Render logs, with `gpt-image-1` working: `3 of FAM's 3
house-style references are missing from /app/visual_references`, and
`no approved references in /app/visual_references — the house style is being
described in words only`.

The packaging was innocent all the way down, which is what made this take
longer than it should have to see:

* `.gitignore` carries a paragraph saying `visual_references/` is deliberately
  **not** ignored, and `git check-ignore` agrees — nothing ignores the files.
* `.dockerignore` excludes credentials, listener data, voices and virtualenvs.
  No pattern touches the folder or `*.png`.
* `Dockerfile` does `COPY . .` into `/app`, and `render.yaml` points Render at
  that Dockerfile with `autoDeploy: true`.

The files themselves were the missing link. `git ls-tree` over **every branch
on the remote** returns `visual_references/README.md` and nothing else: the
three approved illustrations exist in a working copy on a Mac and in no commit
anywhere. Render builds from a clone; a clone carries what was committed; so
`/app/visual_references` held a README, the loader found none of the three
stems, and the whole thing degraded exactly as designed — a warning per
request, `references_missing` on `/api/health`, and art that is visibly not
FAM's.

The gap this really names is between a rule and a fact. The README says "they
are committed" and the ignore file explains why they must be; both were
written by sessions running in containers that never had the artwork, and
nothing ever asked git whether the sentence was true. So
`tests/test_reference_packaging.py` asks git: every stem in `ACTIVE_REFERENCES`
must be a **tracked** file, present on disk is not enough, and where there is
no git to ask (an image, a tarball) it falls back to the only evidence there
is. It also asserts that no ignore rule can quietly take the folder back — a
`*.png` added later for some other reason would otherwise remove the house
style from every future deployment without a line of code changing.

That test is red until the artwork is committed, and that is the correct
colour: while it is red, the repository cannot produce a deployment that draws
in FAM's style. Verified both ways — it fails here naming the three stems and
the exact commands, and passes in a scratch clone the moment three files with
those names are committed. Verified too that the runtime half was never in
doubt: with the files in place, `/api/health` reports all three in force,
`references_missing: []`, and no note, without a restart.
