...
def from_web_search(message) -> Provenance:
    """What the model's own `web_search` tool actually read.

    **The gap this closes** (§107): provenance was built from the Exa packet,
    from live facts, and from attachments - and from nothing else. But FAM has
    a second, entirely ordinary way of researching an episode: when no evidence
    packet comes back, `_request_kwargs` attaches Anthropic's `web_search` tool
    and the model does the looking itself. That is not an edge case. It is
    every episode on a deployment with no Exa key, which is what the Render
    service has always been - so the sources panel was hidden on every episode
    it served, and the reason looked like a rendering bug.

    The results are on the final message rather than in the text stream: a
    server tool runs inside the model's turn and reports back as a
    `web_search_tool_result` content block, which `stream.get_final_message()`
    already returns for the usage accounting beside this.

    Defensive about shape on purpose. These are provider objects whose fields
    may be attributes or dict keys depending on the SDK version, and a
    provenance panel is the last thing in this app that should be able to take
    an episode down - so anything unreadable is skipped rather than raised on.
    """
    out = Provenance()
    for block in _blocks(message):
        ...
        # An error block has `content` as an object rather than a list, and it
        # means the search did not happen. Nothing to attribute.
        if not isinstance(content, (list, tuple)):
            continue
        for result in content:
            url = str(_field(result, "url") or "")
            host = clean_host(url)
            if not host:
                continue
            out.add(Attribution(
                label=host,
                kind=ARTICLE,
                # No grade. `research.credibility` reads an Exa result's own
                # fields, and a web_search result carries none - inventing a
                # tier here would put a confidence on the panel that nothing
                # measured. Blank renders as no grade, which is true.
                tier="",
                # `page_age` is the provider's words ("2 days ago"), not a
                # date, and `at` is documented as ISO. Left empty rather than
                # parsed into a date that was never stated - the same reason
                # `research.published_at` returns None instead of today.
                at="",
                title=str(_field(result, "title") or "")[:200],
                url=url[:500] if url[:8].lower().startswith(("http://", "https://")) else "",
            ))
    if out.items:
        # Named for what a listener is being told: the model went and looked,
        # rather than an index this app queried. `retrievers` is rendered as
        # "cross-checked across ..." and has to stay honest about which.
        out.retrievers.append("web search")
    return out


def _blocks(message):
    """Content blocks off a message, whatever shape it arrived in."""
    content = _field(message, "content")
    return content if isinstance(content, (list, tuple)) else []


def _field(obj, name):
    """One field, from an attribute or a mapping key. Never raises."""
